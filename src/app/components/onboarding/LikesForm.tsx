...
function LikesForm({ setCurrStep }: { setCurrStep: (step: number) => void }) {
  ...
  function handleSubmit() {
    ...
    localStorage.setItem("likedCategories", JSON.stringify(selectedLiked));
    setError("");
    setCurrStep(1);
  }
  ...
  useEffect(function () {
    ...
    if (saved) {
      setSelectedLiked(JSON.parse(saved));
    }
  }, []);

  function handleSelect(name: string) {
    if (selectedLiked.includes(name)) {
      const newList = selectedLiked.filter((category) => category != name);
      setSelectedLiked(newList);
    } else {
      setSelectedLiked((prev) => [...prev, name]);
    }
  }
  return (
    <div className="max-h-screen flex items-center justify-center flex-col md:p-20 text-center p-10">
      <div className="flex gap-4 flex-col mb-12">
        ...
        <h2 ref={h2Ref} className="text-lg">
          Select the categories that suit you the most.
        </h2>
      </div>
      ...
      <div className="flex items-end justify-end w-full max-w-[80vw]">
        <button
